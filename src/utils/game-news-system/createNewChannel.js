...
export default async function createNewChannel(interaction) {
    try {
        ...
        if (!category || category.type !== ChannelType.GuildCategory) {
            await interaction.reply({ content: `> La catégorie spécifiée est introuvable ou invalide.`, flags: MessageFlags.Ephemeral });
            return;
        }
        ...
        if (existingRole) {
            await interaction.reply({ content: `> ⚠️ Les news pour **${gameName}** ont déja été ajoutés.`, flags: MessageFlags.Ephemeral });
            return;
        }
        ...
        await interaction.reply({ content: `> Le salon du jeu **${gameName}** et son rôle ont été créés avec succès.
            > ⚠️ N'oublie pas d'ajouter le rôle dans "Processus d'accueil->Questions->Questions complémentaires.`, flags: MessageFlags.Ephemeral });
    } catch (error) {
        console.error('Erreur lors de la création du salon ou du rôle :', error);
